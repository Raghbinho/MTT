...
####################### GLOBAL VARIABLES ################################################
PRODUCTS = 'Products\\'
TESTCASES = 'D:\\Users\\G507919\\Desktop\\PFE-version7.0 - Copie\\'
CLASSES = 'Classes\\'
...
def home(request, template_name="home.html", path=CLASSES):
                    ...
                    while (j2 < (int(eltsize)) + 1):
                        ...
                        if (attribut[cpt] != ''):
                            ...
                            while (nbes <= (int(attribut[cpt]) - 1)):
                                ...
                                elementtype = request.POST.get(
                                    "elementtype-" + str(i) + "-" + str(eltsize) + "-" + str(nbes) + "-" + str(nb),
                                    None)
...
def getNameF(request):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        return HttpResponse(simplejson.dumps(
            {'stat': status, 'software': software, 'deviceType': deviceType, 'productName': productName,
             'serialNumber': serialNumber, 'manu': manu,
             'desc': desc, 'eqID': eqID, 'meID': meID, 'meterP': meterP, 'serverA': serverA, 'firmware': firmware,
             'tcpTimeOut': tcpTimeOut,
             'hdlcTimeOut': hdlcTimeOut, 'HDLC_maxT': HDLC_maxT, 'HDLC_maxR': HDLC_maxR, 'HDLC_Adr': HDLC_Adr,
             'bau': bau, 'mode': mode, 'Mas_key': Mas_key,
             'global_key': global_key, 'Authen_key': Authen_key, 'L_key': L_key, 'H_key': H_key, 'serverA': serverA,
...
def generateMethod(request):
    ...
    if nbligne2 == None :
        ...
        rough_string1 = re.sub(r'<class*>', '', tostring(treeAtt))
        rough_string2 = re.sub(r'</class?>', '', rough_string1)
        rough_string3 = re.sub(r'<class id="" version=""?>', '', tostring(root))
        # rough_string4 = re.sub(r'</class?>', '', rough_string3)
        tree102 = rough_string2 + rough_string3
    reparsed = minidom.parseString(tree102)
    textXML = reparsed.toprettyxml(indent="\t")
    textXML = re.sub('\n+', '\n', textXML)
    with open("polls/XMLFilesMethod/filenameM" + str(nbfich2) + ".xml", 'w') as f:
        f.write(textXML )
...
def getName(request):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        return HttpResponse(simplejson.dumps({'stat': str(status['fileContent']), 'dict': dict1, 'length': len(dict1)}),
        ...
        if request.POST.__contains__('content'):
            ...
            if (not os.path.isfile(fileName)):
                ...
                pretty_print = lambda f: '\n'.join(
                    [line for line in xml.dom.minidom.parse(open(f)).toprettyxml(indent=' ' * 2).split('\n') if
                     line.strip()])
...
def parseXML(request):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        return HttpResponse(simplejson.dumps(
            {'stat': status, 'dictDescription': description, 'dictId': dictId, 'dictVersion': dictVersion,
             'methods': methods, 'association': association, 'associationMethods': association}),
                            content_type='application/json')
...
def generateXML(request):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        logger.info(textXML)
        top = ET.Element('dictionnary')
        top.attrib['version'] = version
        logger.info(Description)
        now = datetime.datetime.now()
        currentDate = now.isoformat()
        top.attrib['creationDate'] = currentDate
        object = SubElement(top, "object", api="DLMS", name=Description)
        for article in res.findall('attributes'):
            xml_string = ElementTree.tostring(article)
            object.append(article)
        reparsed1 = tostring(top)
        rough_string = re.sub(r'<delimit?>', '', reparsed1)
        rough_string = re.sub(r'</delimit?>', '', rough_string)
        logger.info(rough_string)
    return HttpResponse(rough_string)
...
def generateProduct(request, template_name="listProduct.html"):
    ...
    for i in range(1, (int(nbclient)) + 1):
        ...
        client = ET.SubElement(clientA, "client", adress=unicode(clientAdress),
                               associationName=unicode(associationName))
...
def editProduct(request, template_name="listProduct.html"):
    ...
    if (NewproductName != OldproductName) | (OldDeviceType != NewDeviceType) | (NewserialNumber != OldSerialNumber) | (
        OldEquipement != Newequipement) | (OldSoft != NewSoft) | (Oldmanu != Newmanu) | (Olddesc != NewDesc) | (
        OldMeterIp != NewMeterIp) \
            | (OldMeterPort != NewMeterPort) | (OldServerAdd != NewServerAdd) | (OldFirmware != NewFirmware) | (
        OldTcpTime != NewTcpTime) | (OldHdlcTime != NewHdlcTime) | (OldHdlcMaxTransmit != NewHdlcMaxTransmit) \
            | (OldHdlcMaxR != NewhdlcMaxReceive) | (OldHdlcAddr != NewhdlcAdress) | (Oldbaudrate != Newbaudrate) | (
        Oldmode != Newmode) | (OldMas_keyt != NewmasterKey) | (Oldglobal_key != NewglobalKey) | (OldAuth != NewAuth) | (
        OldhlsSecret != NewhlsSecret) \
        ...
        os.rename("Products\\" + OldproductName + "\\" + OldproductName + ".xml",
                  "Products\\" + OldproductName + "\\" + NewproductName + ".xml")
        ...
        try:
            ...
            for name in lst:
                ...
                if (os.path.isdir(fn)) & (recent == 'TestCases'):
                    ...
                    if (os.path.isdir(
                            fn) == False) and path == 'D:\Users\G507919\Desktop\PFE-version7.0 - Copie\TestCases':
                        tree['children'].append(dict(name=recent))
...
def generateTest(request, template_name="DictionnaryObject.html"):
    ...
    return render(request, template_name,
                  {'tree': tree, 'action': action, 'product': product, 'access': access, 'firm': firm,
                   'AssociationName': AssociationName, 'check': check})
...
def getAttributes(request, template_name="DictionnaryObject.html"):
    ...
    if request.method == "POST" and request.is_ajax():
        ...
        for a in object:
            ...
            for name in attr:
                ...
                for assoc in dlmsAttribute:
                    ...
                    if j < nbatt - 1:
                        j += 1
                        ch1 += (att.split(','))[j] + ':' + (type.split(','))[j] + ':' + (idAtt.split(','))[
                            j] + "=" + res + "\n"
                        # print (att.split(','))[j]
                break
        ...
        return HttpResponse(
            simplejson.dumps(
                {'att': att, 'meth': meth, 'ident': idAtt, 'idMeth': idMeth, 'type': type, 'path': path, 'pdt': pdt,
                 'action': action, "file": FileName1, 'method_id': method_id,
                 'operation': operation, 'MNGT': MNGT, 'pre': pre, 'public': public, 'getAssocM': getAssocM,
                 'getAssocPu': getAssocPu, 'getAssocPr': getAssocPr, 'ch1': ch1, 'ch2': ch2, 'nbatt': nbatt,
                 'nbmeth': nbmeth}),
            content_type='application/json')
...
def getXML(request, template_name="welcome.html"):
    ...
    if action == "Get":
        ...
        typeValue = ET.SubElement(value1, unicode(AttributChar[1]), name=unicode(AttributChar[0]),
        ...
        if function == "ActionRequest":
            ...
            return HttpResponse(simplejson.dumps(
                {'stat': status, 'identifiant': identifiant, 'dataLinkName': dataLinkName, 'function': function,
                 ...
                 'checkRes': checkRes, 'ok': ok, 'default': default, 'descr': descr, 'prdt': prdt}),
                content_type='application/json')
            ...
            return HttpResponse(simplejson.dumps(
                {'stat': status, 'identifiant': identifiant, 'dataLinkName': dataLinkName, 'function': function,
                 ...
                 'checkRes': checkRes,
                 'descr': descr, 'typeval': typeval, 'valN': valN, 'valV': valV, 'ok': ok, 'default': default,
                 'prdt': prdt}), content_type='application/json')
